...
[TestClass]
public class ScapegoatTreeRecordStoreTests
{
    ScapegoatTreeRecordStore store_ = new();
    ...
    public void Remove_ShouldWork()
    {
        ...
        var values3 = values1.Where(n => !values2.Contains(n)).Order().ToArray();
        var values4 = store_.Enumerate().Select(n => n.Item1).Order().ToArray();
        var result = values3.SequenceEqual(values4);
    ...
    {
        var store1 = new ScapegoatTreeRecordStore();
        var values1 = new List<(int, int)>();
        for (var i = 0; i < 10000; i++)
        {
        ...
        var store2 = BasicRecordStore.Deserialize<ScapegoatTreeRecordStore>(serialized);
        var values2 = new List<(int, int)>();
