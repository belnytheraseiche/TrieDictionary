...
[TestClass]
public class LoudsDictionaryTests
{
    ...
    public void Create_WithKeys_ShouldWork()
    {
        ...
        var dictionary = KeyRecordDictionary.Create<LoudsDictionary>(keys);
    ...
    public void Serialize_Deserialize_ShouldWork()
    {
        ...
        List<byte[]> bytes = [.. keys.Select(n => ToBytes(n))];
        var dictionary1 = KeyRecordDictionary.Create<LoudsDictionary>(keys);
        foreach (var key in bytes)
            dictionary1.GetRecordAccess(dictionary1.SearchExactly(key)).Add(key);

        var serialized1 = KeyRecordDictionary.Serialize<LoudsDictionary>(dictionary1);
        var dictionary2 = KeyRecordDictionary.Deserialize<LoudsDictionary>(serialized1);
        List<string> records = [];
    ...
    public void Serialize_With_Options_ShouldWork()
    {
        ...
        var serialized1 = KeyRecordDictionary.Serialize<LoudsDictionary>(dictionary1, options);
        var dictionary2 = KeyRecordDictionary.Deserialize<LoudsDictionary>(serialized1);
        List<string> records = [];
        foreach (var (identifier, key) in dictionary2.EnumerateAll())
        {
    ...
    {
        var dictionary = LoudsDictionary.Create([ToBytes("a")], KeyRecordDictionary.SearchDirectionType.LTR);
    ...
    public void GetKey_ShouldWork()
    {
        ...
        var dictionary = LoudsDictionary.Create([bytes], KeyRecordDictionary.SearchDirectionType.LTR);
    ...
    public void EnumerateAll_ShouldReturnAllKeysInOrder()
    {
        ...
        var dictionary = LoudsDictionary.Create(keys.Select(n => ToBytes(n)), KeyRecordDictionary.SearchDirectionType.LTR);
        var result = dictionary.EnumerateAll().Select(n => ToString(n.Item2)).ToList();
        CollectionAssert.AreEquivalent(result, keys);
    }
    ...
    public void SearchLongestPrefix_ShouldWork()
    {
        ...
        var dictionary = KeyRecordDictionary.Create<LoudsDictionary>(keys, Encoding.ASCII);
    ...
    public void Rtl_SearchExactly_ShouldWork()
    {
        ...
        var dictionary = KeyRecordDictionary.Create<LoudsDictionary>(keys, Encoding.ASCII, KeyRecordDictionary.SearchDirectionType.RTL);
