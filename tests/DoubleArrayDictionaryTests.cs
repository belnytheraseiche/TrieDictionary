...
[TestClass]
public class DoubleArrayTrieDictionaryTests
{
    DoubleArrayDictionary dictionary_ = KeyRecordDictionary.Create<DoubleArrayDictionary>(KeyRecordDictionary.SearchDirectionType.LTR);
    ...
    public void Create_WithKeys_ShouldWork()
    {
        ...
        var dictionary = KeyRecordDictionary.Create<DoubleArrayDictionary>(keys);
    ...
    public void Serialize_Deserialize_ShouldWork()
    {
        ...
        List<byte[]> bytes = [.. keys.Select(n => ToBytes(n))];
        var dictionary1 = KeyRecordDictionary.Create<DoubleArrayDictionary>(keys);
        foreach (var key in bytes)
            dictionary1.GetRecordAccess(dictionary1.SearchExactly(key)).Add(key);

        var serialized1 = KeyRecordDictionary.Serialize<DoubleArrayDictionary>(dictionary1);
        var dictionary2 = KeyRecordDictionary.Deserialize<DoubleArrayDictionary>(serialized1);
        List<string> records = [];
    ...
    public void Serialize_With_Options_ShouldWork()
    {
        ...
        var serialized1 = KeyRecordDictionary.Serialize<DoubleArrayDictionary>(dictionary1, options);
        var dictionary2 = KeyRecordDictionary.Deserialize<DoubleArrayDictionary>(serialized1);
        List<string> records = [];
        foreach (var (identifier, key) in dictionary2.EnumerateAll())
        {
    ...
    [TestMethod]
    public void Add_And_GetKey_ShouldWork()
    {
        var key = "test";
        var bytes = ToBytes(key);
        var identifier = dictionary_.Add(bytes);
        var retrieved = dictionary_.GetKey(identifier);
        Assert.IsGreaterThan(0, identifier);
        CollectionAssert.AreEquivalent(retrieved, bytes);
        Assert.IsTrue(dictionary_.Contains(bytes));
    }

    [TestMethod]
    public void Transient_Records_ShouldWork()
    {
        var identifier = dictionary_.Add(ToBytes("a"));
        var access = dictionary_.GetRecordAccess(identifier, true);
    ...
    {
        var identifier = dictionary_.Add(ToBytes("a"));
        var access = dictionary_.GetRecordAccess(identifier, false);
        access.Add(ToBytes("abc")).InsertAfter(ToBytes("abe")).InsertBefore(ToBytes("abd"));
        var record0 = access.Add(ToBytes("abf"));
        List<string> result1 = [];
        foreach (var record1 in access)
    ...
    [TestMethod]
    public void Identifier_RemainsValid_AfterRelocation()
    {
        var identifier1 = dictionary_.Add(ToBytes("a"));
        var identifier2 = dictionary_.Add(ToBytes("b"));
        var identifier3 = dictionary_.Add(ToBytes("aa"));
        CollectionAssert.AreEqual(dictionary_.GetKey(identifier1), ToBytes("a"));
        CollectionAssert.AreEqual(dictionary_.GetKey(identifier2), ToBytes("b"));
        CollectionAssert.AreEqual(dictionary_.GetKey(identifier3), ToBytes("aa"));
    }

    [TestMethod]
    public void GetKey_WithStaleIdentifier_ShouldResolve_AfterRelocation()
    {
        var identifier = dictionary_.Add(ToBytes("cat"));
        dictionary_.Add(ToBytes("catalog"));
        dictionary_.Add(ToBytes("car"));
        CollectionAssert.AreEqual(dictionary_.GetKey(identifier), ToBytes("cat"));
    }

    [TestMethod]
    public void Remove_ByKey_ShouldWork()
    {
        var key = ToBytes("removable");
        dictionary_.Add(key);
        Assert.IsTrue(dictionary_.Contains(key));
        var removed = dictionary_.Remove(key);
        Assert.IsFalse(dictionary_.Contains(key));
        Assert.IsTrue(removed);
    }

    [TestMethod]
    public void Remove_ByIdentifier_ShouldWork()
    {
        var key = ToBytes("removable");
        var identifier = dictionary_.Add(key);
        Assert.IsTrue(dictionary_.Contains(key));
        var removed = dictionary_.Remove(identifier);
        Assert.IsTrue(removed);
        Assert.IsFalse(dictionary_.Contains(key));
    }

    [TestMethod]
    public void Remove_ShouldAlsoRemoveRecords()
    {
        var key = ToBytes("keywithrecords");
        var identifier1 = dictionary_.Add(key);
        var access1 = dictionary_.GetRecordAccess(identifier1, false);
        access1.Add(ToBytes("persistent record"));
        var access2 = dictionary_.GetRecordAccess(identifier1, true);
        access2.Add(ToBytes("transient record"));
        Assert.AreEqual(access1.Count(), 1);
        Assert.AreEqual(access2.Count(), 1);

        dictionary_.Remove(key);
        Assert.IsFalse(dictionary_.Contains(key));

        var identifier2 = dictionary_.Add(key);
        var access3 = dictionary_.GetRecordAccess(identifier2, false);
        var access4 = dictionary_.GetRecordAccess(identifier2, true);
        Assert.AreEqual(access3.Count(), 0);
        Assert.AreEqual(access4.Count(), 0);
    }

    [TestMethod]
    public void Compact_ShouldCleanupOrphanedForwardingPointers()
    {
        var identifier1 = dictionary_.Add(ToBytes("a"));
        dictionary_.Add(ToBytes("aa"));
        dictionary_.Remove(identifier1);
        Assert.IsFalse(dictionary_.Contains(ToBytes("a")));
        Assert.IsTrue(dictionary_.Contains(ToBytes("aa")));

        var compact = DoubleArrayDictionary.Compact(dictionary_);
        Assert.IsFalse(compact.Contains(ToBytes("a")));
        Assert.IsTrue(compact.Contains(ToBytes("aa")));

        var identifier2 = compact.SearchExactly(ToBytes("aa"));
        Assert.IsGreaterThan(0, identifier2);
        CollectionAssert.AreEqual(compact.GetKey(identifier2), ToBytes("aa"));
    }

    [TestMethod]
    public void EnumerateAll_ShouldReturnAllKeysInOrder()
    {
        dictionary_.Add(ToBytes("c"));
        dictionary_.Add(ToBytes("a"));
        dictionary_.Add(ToBytes("b"));
        var result = dictionary_.EnumerateAll().Select(n => Encoding.UTF8.GetString(n.Item2)).ToList();
        CollectionAssert.AreEquivalent(result, (string[])["a", "b", "c"]);
    }

    [TestMethod]
    public void TryAdd_ShouldWork()
    {
        var identifier0 = dictionary_.Add(ToBytes("a"));
        var added1 = dictionary_.TryAdd(ToBytes("a"), out var identifier1);
        var added2 = dictionary_.TryAdd(ToBytes("b"), out var identifier2);
        Assert.IsFalse(added1);
        Assert.IsTrue(added2);
        Assert.AreEqual(identifier1, identifier0);
    }

    [TestMethod]
    public void Contains_ShouldWork()
    {
        dictionary_.Add(ToBytes("abc"));
        var result1 = dictionary_.Contains(ToBytes("abc"));
        var result2 = dictionary_.Contains(ToBytes("ab"));
    ...
    {
        var identifier0 = dictionary_.Add(ToBytes("abc"));
        var identifier1 = dictionary_.SearchExactly(ToBytes("abc"));
        var identifier2 = dictionary_.SearchExactly(ToBytes("abcd"));
        Assert.AreEqual(identifier1, identifier0);
    ...
    public void SearchCommonPrefix_SearchLongestPrefix_SearchByPrefix_ShouldWork()
    {
        ...
        foreach (var key in keys)
            dictionary_.Add(ToBytes(key));
        var result1 = dictionary_.SearchCommonPrefix(ToBytes("appstart")).Select(n => ToString(n.Item2)).Order().ToArray();
        var result2 = ToString(dictionary_.SearchLongestPrefix(ToBytes("appstart")).Item2);
        var result3 = dictionary_.SearchByPrefix(ToBytes("ap")).Select(n => ToString(n.Item2)).Order().ToArray();
    ...
    public void SearchLongestPrefix_ShouldWork()
    {
        ...
        var dictionary = KeyRecordDictionary.Create<DoubleArrayDictionary>(keys, Encoding.ASCII);
    ...
    public void SearchWildcard_ShouldWork()
    {
        ...
        var dictionary = DoubleArrayDictionary.Create(keys.Select(n => ToBytes(n)), KeyRecordDictionary.SearchDirectionType.LTR);
    ...
    public void FindFirst_FindNext_FindLast_FindPrevious_ShouldWork()
    {
        ...
        foreach (var key1 in keys)
            dictionary_.Add(ToBytes(key1));
        var result1 = new List<string>();
        if (dictionary_.FindFirst(out var identifier2, out var key2))
        {
            result1.Add(ToString(key2));
            while (dictionary_.FindNext(key2, out identifier2, out key2))
                result1.Add(ToString(key2));
        }
        var result2 = new List<string>();
        if (dictionary_.FindLast(out var identifier3, out var key3))
        {
            result2.Add(ToString(key3));
            while (dictionary_.FindPrevious(key3, out identifier3, out key3))
                result2.Add(ToString(key3));
        }
    ...
    public void Rtl_Add_Remove_ShouldWork()
    {
        var keys = (string[])["a", "ab", "abc"];
        var dictionary = KeyRecordDictionary.Create<DoubleArrayDictionary>(keys, Encoding.ASCII, KeyRecordDictionary.SearchDirectionType.RTL);
        var ss = dictionary.AsStringSpecialized(Encoding.ASCII);
        ss.Add("xyz");
        var result1 = ss.Contains("xyz");
        ss.Remove("xyz");
        var result2 = ss.Contains("xyz");
        var result3 = ss.TryAdd("ab", out var _);
        Assert.IsTrue(result1);
        Assert.IsFalse(result2);
